...
class Done
{
  ...
  {
    $query = $dataBaseConnection->prepare($sqlStatement);
    if (count($param) === 1 && is_array($param[0]))
      $query->execute([...$param[0]]);
    else
      $query->execute($param);
  ...
  public static function auth($authCondition, $ifAccessDeniedURL)
  {
    if (!$authCondition) {
      header("location: $ifAccessDeniedURL");
      die();
    }
  }
}
